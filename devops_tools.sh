...
case $tool in
 ...
 docker_install
  ...
  echo "Now restart your instance, to make the changes get effective"
  fi
  else
          ...
          "kubernetes"|"Kubernetes"|"KUBERNETES"|"14")
                ...
                supported_version=20.04.6
                  ...
                  read -p "You are setting up ${bold}Kubernetes$nobold - " decision
                    ...
                    then
                    echo "${bold}Are you trying to install kubernetes master in t2.medium instance or not?${nobold}"
                    read -p "Please confirm  ${bold}yes$nobold or ${bold}no${nobold}- " decision
                      ...
                      echo $USER | grep "root" > /dev/null 2>&1
                        ...
                        docker --version > /dev/null 2>&1
                          ...
                          export KUBECONFIG=/etc/kubernetes/admin.conf
                          touch command
                          echo "export KUBECONFIG=/etc/kubernetes/admin.conf" >as_root_command
                          echo -e "mkdir -p $HOME/.kube\nsudo cp -i /etc/kubernetes/admin.conf $HOME/.kube/config\n\
sudo chown $(id -u):$(id -g) $HOME/.kube/config" >as_normal_user
                          sleep 10
                          ...
                          echo "If you want to check again node status, please execute the below command"
                          ...
                          echo "$version"
                          export KUBECONFIG=/etc/kubernetes/admin.conf
                          touch as_root_command
                          echo "export KUBECONFIG=/etc/kubernetes/admin.conf" >as_root_command
                          echo -e "mkdir -p $HOME/.kube sudo cp -i /etc/kubernetes/admin.conf $HOME/.kube/config\n\
sudo chown $(id -u):$(id -g) $HOME/.kube/config" >as_normal_user
                          sleep 10
                          echo "Kubernetes master status getting ready, please wait ..."
                          kubectl get nodes
                      ...
                      echo "${bold}First launch a t2.medium server in AWS for kubernetes master configuration purpose$nobold"
                      fi               
                 else
                 echo $USER | grep "root" > /dev/null 2>&1
                 if [ $? -ne 0 ]
...
tput bold
echo "Please execute this script in Ubuntu instance, as script supports only apt package"
tput sgr0
fi


free -h | grep Mem | awk '{print substr( $2 , 1, length($2)-2 )}'