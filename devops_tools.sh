    ...
    /etc/apt/sources.list.d/jenkins.list > /dev/null
     ...
     "docker"|"Docker"|"DOCKER"|"5")
                ...
                docker --version | grep "ubuntu" > /dev/null 2>&1
                 if [ $? -eq 0 ]
                 then
                 echo "In your system Un-official $tool version was available and it's Version is"
                 docker --version
                 sleep 2
                 echo "Be cautious !!!! If you proceed further this un-official $tool version will be un-installed"
                 ...
                 echo "To cancel $tool installation press $bold ctrl+c $nobold or wait for official $tool version installation" 
                 sleep 30 
                 echo "Un-official $tool version is being un-installed and official version will be installed in the next following steps"
...
echo \
  ...
  $(. /etc/os-release && echo "$VERSION_CODENAME") stable" | \
        ...
        "grafana"|"Grafana"|"GRAFANA"|"4")
                ...
                echo
                echo "$tool URL generation under progress"
                 if [ $? -eq 0 ]
                 then
      ...
      "sonarqube"|"Sonarqube"|"SONARQUBE"|"11")
                ...
                if [ $? -eq 0 ]
                then
                echo "$tool is available in your server"
                ...
                echo "${bold}$tool server requires minimum system configuration of vCPU's-02 No's and RAM-4 GB (t2.medium)${nobold}"
                echo -e "${bold}Are you trying to install $tool server in t2.medium instance or not${nobold}?"
                yes=1
                no=2
                read -p "Please confirm  ${bold}1 for yes$nobold or ${bold}2 for no${nobold}- " decision
                 ...
                 #echo "Now you have to set the password for 'sonar' user, which will be used to login"
                 #sudo passwd sonar 
...
LimitNOFILE=65536\n LimitNPROC=4096\n [Install]\n WantedBy=multi-user.target\n" | sudo tee -a /etc/systemd/system/sonar.service > /dev/null 2>&1
                 ...
                 curl --connect-timeout 4 http://$sonarqube_ip:$sonarqube_port > /dev/null 2>&1
                  if [ $? -eq 0 ]
                  then
                  echo "$tool was installed sucessfully and you can browse using following URL"
                 ...
                 then
                 echo "First launch a t2.medium server in AWS for $tool server installation"
                 else
      ...
      "node_exporter"|"Node_exporter"|"NODE_EXPORTER"|"13")
                ...
                echo "Please wait $tool server URL generation under progress it takes less than 40 Sec's"
                sleep 40
                sudo curl --connect-timeout 4 http://$node_exporter_ip:$node_exporter_port  > /dev/null 2>&1
                 ...
                 echo "$tool version:- $bold$node_version$nobold"
...
WantedBy=multi-user.target\n " | sudo tee -a /etc/systemd/system/node_exporter.service > /dev/null 2>&1
     ...
     "prometheus"|"Prometheus"|"PROMETHEUS"|"12")
      ...
      "jfrog"|"Jfrog"|"JFROG"|"7")
                ...
                echo "Please wait $tool server URL generation under progress it takes less than a 55 sec's "
                sleep 54
                jfrog_port=8081              
                sudo curl --connect-timeout 4 http://$jfrog_ip:$jfrog_port  > /dev/null 2>&1
                 ...
                 echo "You need to allow the port number$bold $jfrog_port$nobold in security groups"
                 fi
                else
                echo "${bold}$tool server requires minimum system configuration of vCPU's-02 No's and RAM-4 GB (t2.medium)${nobold}"
                echo -e "${bold}Are you trying to install $tool server in t2.medium instance or not${nobold}?"
...
[Install]\n WantedBy=multi-user.target\n" | sudo tee /etc/systemd/system/artifactory.service > /dev/null 2>&1
                 ...
                 echo "Please wait $tool server URL generation under progress wait for 55 sec's only"
                 sleep 55
