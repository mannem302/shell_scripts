...
tput bold
echo "1) Git               8) Tomcat          15) Exit from this Script"
...
version=`curl http://localhost:$tomcat_port/manager/html -u ${tomcat_user}:${tomcat_password} | grep "Apache Tomcat" | cut -d ">" -f 3 | cut -d "<" -f 1 | cut -d " " -f 1,2`
  ...
  case $tool in
    "tomcat"|"Tomcat"|"TOMCAT"|"8" )
           ...
           tomcat_port=`sudo cat server.xml | grep port= | cut -d '=' -f 2 | cut -d '"' -f 2 | head -2 | tail -1` > /dev/null 2>&1
	         tomcat_timer_6
           sudo curl --connect-timeout 2 http://localhost:$tomcat_port | grep tomcat > /dev/null 2>&1 
             if [ $? -eq 0 ]
             then
             sudo curl --connect-timeout 2 http://$tomcat_ip:$tomcat_port | grep tomcat > /dev/null 2>&1
              ...
              tomcat_version
              tput sgr0
              echo "Now you can open browser ${bold}http://localhost:$tomcat_port $nobold"
              echo "Your Tomcat-Server username is $bold'$tomcat_user'$nobold and password is $bold'$tomcat_password'$nobold"
              echo "Tomcat is working as expected"
              fi
             else 
           ...
           sudo curl --connect-timeout 2 http://localhost:$tomcat_port | grep tomcat > /dev/null 2>&1
             ...
             echo "Now you can open browser using the URL ${bold}http://localhost:$tomcat_port${nobold}"
             echo "Your Tomcat-Server username is $bold'$tomcat_user'$nobold and password is $bold'$tomcat_password'$nobold"
             fi
             else
             sudo curl --connect-timeout 2 http://localhost:$tomcat_port | grep tomcat > /dev/null 2>&1
             ...
             alloted_port=`sudo cat /lib/systemd/system/jenkins.service | grep "JENKINS_PORT" | cut -d "=" -f 3 | cut -d '"' -f 1`
               ...
               sudo curl --connect-timeout 4 http://localhost:$tomcat_port | grep tomcat > /dev/null 2>&1
                 if [ $? -eq 0 ] 
                 then
    ...
    "jenkins"|"Jenkins"|"JENKINS"|"6")
                ...
                sudo systemctl restart $service
                jenkins_timer_70
                sudo curl --connect-timeout 2 http://localhost:$jenkins_port  > /dev/null 2>&1
                  if [ $? -eq 0 ]
                  then
                  ...
                  echo "Open your browser and Enter 'localhost' and port number is $jenkins_port"
                  echo "You have to execute like this in your browser ${bold}http://localhost:$jenkins_port$nobold "
                  echo "Jenkins server is working as expected"
                  fi
                  else
                  ...
                  jenkins_port=`sudo cat /lib/systemd/system/jenkins.service | grep "JENKINS_PORT" | cut -d "=" -f 3 | cut -d '"' -f 1` 
                    ...
                    sudo systemctl restart $service
	                  jenkins_timer_130
                    jenkins_port=`sudo cat /lib/systemd/system/jenkins.service | grep "JENKINS_PORT" | cut -d "=" -f 3 | cut -d '"' -f 1`
                    sudo curl --connect-timeout 2 http://localhost:$jenkins_port | grep jenkins > /dev/null 2>&1
                      if [ $? -eq 0 ] 
                      then
    ...
    /etc/apt/sources.list.d/jenkins.list > /dev/null
                ...
                alloted_port=`sudo cat /var/lib/tomcat*/conf/server.xml | grep port= | cut -d '=' -f 2 | cut -d '"' -f 2 | head -2 | tail -1` > /dev/null 2>&1
                jenkins_timer_10
                sudo curl --connect-timeout 2 http://localhost:$jenkins_port | grep Authentication > /dev/null
                 ...
                 jenkins_port=`sudo cat /lib/systemd/system/jenkins.service | grep "JENKINS_PORT" | cut -d "=" -f 3 | cut -d '"' -f 1` > /dev/null 2>&1
                 jenkins_timer_130
                 sudo curl --connect-timeout 4 http://localhost:$jenkins_port | grep Authentication > /dev/null 2>&1
                  if [ $? -eq 0 ]
                  then
...
[Install]\n WantedBy=multi-user.target\n" | sudo tee /etc/systemd/system/artifactory.service > /dev/null 2>&1
          ...
          "kubernetes"|"Kubernetes"|"KUBERNETES"|"14")
                ...
                ;;
	        15) echo "You are exiting from the script"
		    break;;
                *)
